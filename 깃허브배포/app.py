...
#=====================================================================================================
# 1. 설정 및 디자인
# 안형준: 실제 사용자와 맞닿을 웹 페이지의 user interface의 디자인적 요소를 설정하는 구획이다. 
#        코드 담당자(안형준)은 웹 페이지 개발에는 전무한 지식을 가지고 있었으므로 이 부분은 생성형 ai의 도움을 받았다.        
...
# 안형준: 이 구획은 UI 화면을 구성하는 텍스트와 부상 부위에 대한 진단, NRS 수치에 따른 처치법 데이터를 저장하는 구획이다. 
#        한국어를 비롯한 영어 일본어 중국어 데이터를 딕셔너리 형태로 저장하고 있으며, 데이터 구성은 생성형ai의 도움을 받았다. 
...
# 안형준: 사용자에게 띄울 자가처치, 응급처지를 담당하는 action 데이터는 mild / mod / emerg 3단계로 분리하여 
#        사용자가 입력하는 NRS 수치에 따라 알맞은 단계의 action 데이터가 뜨도록 하였다. 
