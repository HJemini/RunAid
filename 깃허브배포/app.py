...
# 2. 다국어 텍스트 데이터 (번역 DB)
# ==========================================
# ... (LANG_TEXT 내용은 이전과 동일하여 생략, 그대로 사용하세요) ...
LANG_TEXT = {
    "한국어": {
        "title": "RunAid",
    ...
    # ... (中文, 日本語 생략 - 이전 코드와 동일하게 유지) ...
    "中文": { "title": "RunAid", "loc_header": "1️⃣ 确认当前位置", "loc_info": "点击下方按钮获取GPS信息（需允许浏览器权限）。", "loc_success": "📍 位置确认完毕！", "loc_warn": "需要获取位置信息才能推荐医院。", "body_header": "2️⃣ 输入受伤信息", "body_label": "请选择疼痛部位", "nrs_header": "3️⃣ 疼痛程度 (NRS)", "nrs_guide_cap": "💡 NRS: 数字越大，疼痛越严重。", "nrs_label": "请选择疼痛分数 (0 ~ 10)", "btn_search": "查找医院 & 开始诊断", "err_loc": "请先点击上方按钮获取位置信息！", "res_header": "🔄 分析结果", "msg_mild": "轻微疼痛。", "msg_mild_tip": "护理建议", "msg_mild_sub": "为了以防万一，为您介绍附近的医院。", "msg_warning": "需要专科医生诊疗。", "msg_warning_sub": "建议去医院就诊，而不是自行处理。", "msg_emerg": "需要立即采取措施的紧急情况！", "msg_emerg_sub": "请不要移动。必须立即去急诊室。", "call_119": "📞 拨打 119", "hosp_header": "🏥 最近的医院 / 韩医院", "cat_ortho": "🦴 [骨科]", "cat_orient": "🌿 [韩医院]", "info_map": "查看地图", "no_data": "附近无信息" },
    "日本語": { "title": "RunAid", "loc_header": "1️⃣ 現在地の確認", "loc_info": "下のボタンを押してGPS情報を取得します（ブラウザの権限許可が必要）。", "loc_success": "📍 位置確認完了！", "loc_warn": "位置情報を取得しないと病院を推薦できません。", "body_header": "2️⃣ 怪我情報の入力", "body_label": "痛む部位を選択してください", "nrs_header": "3️⃣ 痛みの程度 (NRS)", "nrs_guide_cap": "💡 NRS: 数字が大きいほど痛みが強いことを意味します。", "nrs_label": "痛みのスコアを選択 (0 ~ 10)", "btn_search": "病院検索 & 診断開始", "err_loc": "先に上のボタンを押して位置情報を取得してください！", "res_header": "🔄 分析結果", "msg_mild": "軽度の痛みです。", "msg_mild_tip": "ケアのヒント", "msg_mild_sub": "万が一のために近くの病院を案内します。", "msg_warning": "専門医の診療が必要です。", "msg_warning_sub": "自己処置より病院の受診をお勧めします。", "msg_emerg": "早急な措置が必要な緊急事態です！", "msg_emerg_sub": "動かないでください。直ちに救急室へ行く必要があります。", "call_119": "📞 119番にかける", "hosp_header": "🏥 最寄りの病院 / 韓医院", "cat_ortho": "🦴 [整形外科]", "cat_orient": "🌿 [韓医院]", "info_map": "地図", "no_data": "近くの情報なし" }
}

# ... (INJURY_DATA는 이전과 동일하여 생략, 그대로 사용하세요) ...
INJURY_DATA = {
...
# (3) 통증 점수 선택 (이미지 추가된 부분!)
st.subheader(txt["nrs_header"])

# ▼▼▼ 여기에 이미지가 들어갑니다 ▼▼▼
current_path = os.path.dirname(os.path.abspath(__file__))
img_path = os.path.join(current_path, "image_0.png") # 이미지 파일명 확인!

if os.path.exists(img_path):
    # use_column_width=True로 설정하면 화면 너비에 맞춰 이미지가 커집니다.
    st.image(img_path, caption=txt["nrs_guide_cap"], use_column_width=True)
else:
    # 이미지가 없을 경우 텍스트로 대체
    st.info("ℹ️ NRS(Numerical Rating Scale): 0(통증 없음) ~ 10(상상할 수 있는 최악의 통증)")
